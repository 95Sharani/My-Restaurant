...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
        ...
        var Draw = {
            columns: {
                ...
                toolbar: function() {
                    if (settings.editButton || settings.deleteButton || settings.printButton) {
                        var editButton = '';
                        var deleteButton = '';
                        var printButton = '';
                        var saveButton = '';
                 ...
                 /*       // Create print button.
                        if (settings.printButton) {
                            printButton = '<button type="button" class="tabledit-print-button  ' + settings.buttons.print.class + '" style="float: none;">' + settings.buttons.print.html + '</button>';
                        ...
                        var toolbar = '<div class="tabledit-toolbar ' + settings.toolbarClass + '" style="text-align: left;">\n\
                                           <div class="' + settings.groupClass + '" style="float: none;">' + editButton + deleteButton + printButton + '</div>\n\
                                           ' + saveButton + '\n\
                                           ' + confirmButton + '\n\
                                           ' + restoreButton + '\n\
                                       </div></div>';
        ...
        var Print = {
            ...
            submit: function(td) {
                // Send AJAX request to server.
                var ajaxResult = ajax(settings.buttons.print.action);
        ...
        function ajax(action)
        {
            ...
            var jqXHR = $.post(settings.url, serialize, function(data, textStatus, jqXHR) {
                ...
                if (action === settings.buttons.print.action) {
                    $lastEditedRow.removeClass(settings.dangerClass).addClass(settings.warningClass);
                    setTimeout(function() {
                        //$lastEditedRow.removeClass(settings.warningClass);
                        $table.find('tr.' + settings.warningClass).removeClass(settings.warningClass);
  ...
  /*     if(settings.printButton){
            $table.on('click', 'button.tabledit-print-button', function(event){
                if (event.handled !== true) {
                    event.preventDefault();
             ...
             // Get table row.
              ...
              // Get input element.
               // var $input = $(td).find('.tabledit-input');
               Print.submit($(this).parents('td'));
              //  window.location= 'FullPayment.php';
                event.handled = true;
            }
            });
        } */
